...
2.  Therefore, we would like to first assess the linear relationship between each of potential covariates and the response variable `MEDV`
    1.  For continuous variables
        ...
        2.  we then used scatterplots between each of the continyous covariates and the response variable `MEDV` to visualize the relationship and assess the linearity. Out of which, the plots between `DIS` and our response variable `MEDV` is mostly random with no obvious linear relationship (nor higher power linearity)
