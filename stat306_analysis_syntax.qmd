...
2.  The origional dataset contains 22 rows of metadata and no headers, we skipped 22 rows when reading the data and we named it `data` and added headers afterwards using `colnames(data)`
...
2.  The output of `descrive` is a table with rows as the variables in our data and columns representing the summary statistics of each variables. In this table, we mainly looks at the `mean`, `sd`, `min`, `max`,`range`, and there are no particular irregular patterns of concerns
3.  In this dataset, we have 14 variables in total and 12 of which are pure continous variables
    1.  `RAD`: how easy to access highways from the neighborhood being a [ordinal varianle,]{.underline} since it has 24 "levels" ordered by how easy it is to access highway from a neighborhood, by the principle of parsimony, we decided to consider `RAD` as a continous variables.
    2.  `CHAS`: is a categorical variables representing if land touches Charles River, which is represented as a dummy variables (1 if yes; 0 otherwise)
...
2.  Therefore, we would like to first assess the linear relationship between each of potential covariates and the response variable `MEDV`
    1.  For continous variables
        ...
        2.  we then used scaterplots between each of the continous covariates and the response variable `MEDV` to visualize the relationship and assess the linearity. Out of which, the plots between `DIS` and our reponse vaeriable `MEDV` is mostly random with no obvious linear relationship (nor higher power linearity)
    ...
    2.  For categorical variable
        1.  we use a boxplot to assess if there is difference between the two categories, and from the boxplot, we see that the Median prices of the neighborhood that is near Charles lake (`CHAS` = 1) is in general higher than the neighborhoods that is not near Charles lake (`CHAS` = 0). So it is meaningfule to include `CHAS` in a linear model.
...
1.  We suspect that some of the continous variables might be similar to each other and cause a multicollinearity issue of our linear model, therefore we first do a correlation tables with upper diagonal being the correlation values and the lower diagonal being the scaterplots using `ggpairs` function from `GGally` package.
    1.  from the result tables, we observe that there are some high pair-wise correlation between the continous covariates, for example, cor(TAX, RAD) = 0.910, cor(NOX, INDUS) = 0.763, cor(NOX, AGE) = 0.737, which signal a potential issue of multicolinearity, therefore we decided to check the Variance Inflation Factor (VIF) in the model (with all potential covariates)
...
suppressWarnings(suppressMessages({
  ...
  plot <- ggpairs(
    ...
    title = "Correlations and Scatterplots Between Continous Covariates"
  )
...
-   From the table above, we observe that the forward final model has a worse $C_p$ than backward final model, but forward final model has a similar adjusted $R^2$ and fewer parameters (covariates) with 5 covariates (compare to 10 in backward selection model), Therefore we decided to use the forward selection model.
...
1.  to assess the model fit of the chosen model, we first did a residual plot
    1.  from the residual plot, we see a cancave up curve pattern for our residual, meaning that there might be some quadratic relationship that was not accounted for by our current model
    2.  Also since there is no obvious constant variance violation, we decided not to do variable transformation at this step
2.  to assess the normality assumption violation, we did a Q-Q plot and observe there is a slightly off the straight line especially in the upper end
...
## Model Selection IV: Consider Quadratic terms

-   From the previous assessment, we observe that there might be quadratic relationship in the residual that were not accounted for by the current model, but we are not sure which covariate to add a quadratic term

-   So we try adding the covariate term on each of the covariate one at a time to see which quadratic terms inprove the model fit the most

-   here is a table summarizing the shape of the residual plot and Q-Q plot for each of the model with qudratic terms

    | Added Quadratic term(s) | $adj R^2$ | $p$ val | Left-over quadratic pattern in residual plot | Normality violation in Q-Q plot |
    |-------------------------|-----------|---------|----------------------------------------------|---------------------------------|
    | $LSTAT^2$               | 0.7406    | 0.000   | moderate                                     | moderate                        |
    | $RM^2$                  | 0.7679    | 0.000   | Low                                          | Low                             |
    | $PTRATIO^2$             | 0.6941    | 0.145   | High                                         | High                            |
    | $B^2$                   | 0.6916    | 0.001   | High                                         | High                            |
    | $LSTAT^2$ & $RM^2$      | 0.7813    | 0.000   | Low (similar to only include $RM^2$)         | Low (similar to only $RM^2$)    |

    -   in summary, from adding the quadratic term for each term one by one, we suspect the quadratic relationship is mostly likely from RM, since after adding RM\^2 term, our residual plot shows a good model fit with quadratic disappear and residuals randomly centered around 0, and the normality violation got better as well. The adjusted R squared is also the highest.

    -   we do notice that including LSTAT\^2 also slightly make it better than the rest of the covariates but not as good as including quadratic RM, so we tried including both on the last model. However, the residual plot and Q-Q plot are very similar to that of only including RM sqrd, and the adjusted R squared are comparable with that of only RM sqrd. So by the principle of parsimony, we decided to just include RM sqrd.

-   At this stage, our model of choice is the model with these covariates:

    -   `LSTAT`: percentage of population in each neighborhood considered lower income

    -   `LSTAT^2`: (squared) percentage of population in each neighborhood considered lower income
...
```{r}
add_quad_LSTAT <- lm(MEDV ~ LSTAT + I(LSTAT^2) + RM + PTRATIO + B + CHAS, data = data)
summary(add_quad_LSTAT)

add_quad_RM <- lm(MEDV ~ LSTAT + RM + I(RM^2) + PTRATIO + B + CHAS, data = data)
summary(add_quad_RM)

add_quad_PTRATIO <- lm(MEDV ~ LSTAT + RM  + PTRATIO + I(PTRATIO^2) + B + CHAS, data = data)
summary(add_quad_PTRATIO)

add_quad_B <- lm(MEDV ~ LSTAT + RM  + PTRATIO + PTRATIO + I(B^2) + CHAS, data = data)
summary(add_quad_B)

add_quad_LSAT_RM <- lm(MEDV ~ LSTAT + + I(LSTAT^2) + RM + I(RM^2) + PTRATIO + B + CHAS, data = data)
summary(add_quad_LSAT_RM)

res_1 <- resid(add_quad_LSTAT)
fitted_1 <- fitted(add_quad_LSTAT)
res_plot_1<-ggplot(data,aes(y=res_1,x=fitted_1))+
  geom_point()+
  ylab("Residuals")+
  xlab("Fitted Values")+
...
res_2 <- resid(add_quad_RM)
fitted_2 <- fitted(add_quad_RM)
res_plot_2<-ggplot(data,aes(y=res_2,x=fitted_2))+
  geom_point()+
  ylab("Residuals")+
  xlab("Fitted Values")+
  ggtitle("Resisduals vs Fitted Values")+
  geom_hline(yintercept = 0)
res_plot_2
qqnorm(res_2, pch = 1, frame = FALSE)
qqline(res_2, col = "green", lwd = 2)

res_3 <- resid(add_quad_PTRATIO)
fitted_3 <- fitted(add_quad_PTRATIO)
res_plot_3<-ggplot(data,aes(y=res_3,x=fitted_3))+
  geom_point()+
  ylab("Residuals")+
  xlab("Fitted Values")+
  ggtitle("Resisduals vs Fitted Values")+
  geom_hline(yintercept = 0)
res_plot_3
qqnorm(res_3, pch = 1, frame = FALSE)
qqline(res_3, col = "green", lwd = 2)

res_4 <- resid(add_quad_B)
fitted_4 <- fitted(add_quad_B)
res_plot_4<-ggplot(data,aes(y=res_4,x=fitted_4))+
  geom_point()+
  ylab("Residuals")+
  xlab("Fitted Values")+
  ggtitle("Resisduals vs Fitted Values")+
  geom_hline(yintercept = 0)
res_plot_4
qqnorm(res_4, pch = 1, frame = FALSE)
qqline(res_4, col = "green", lwd = 2)

res_5 <- resid(add_quad_LSAT_RM)
fitted_5 <- fitted(add_quad_LSAT_RM)
res_plot_5<-ggplot(data,aes(y=res_5,x=fitted_5))+
  geom_point()+
  ylab("Residuals")+
  xlab("Fitted Values")+
  ggtitle("Resisduals vs Fitted Values")+
  geom_hline(yintercept = 0)
res_plot_5
qqnorm(res_5, pch = 1, frame = FALSE)
qqline(res_5, col = "green", lwd = 2)
```

## 9. Model fit diagnosis II

-   for the second step of model fit diagonsis, we plotted sequantial residual to check for violation for independent assumption

    -   from the graph below, the residuals seems to be random and no clear patten, signalling no obvious violation for independent assumption.

```{r}
plot(head(res_2,99), tail(res_2,99),
     xlab="e(i)", ylab="e(i+1)", cex=1.5, cex.lab=1.5, pch=16)
```

## 10. Current Final Model
